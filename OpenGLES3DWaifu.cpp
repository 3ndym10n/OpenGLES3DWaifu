...
class OpenGLES3DWaifu : public pvr::Shell
{
...
void OpenGLES3DWaifu::eventMappedInput(pvr::SimplifiedInput action)
{
	switch (action)
	{
...
pvr::Result OpenGLES3DWaifu::quitApplication()
{
	_scene.reset();
	return pvr::Result::Success;
}
...
/// <returns>Return Result::Success if no error occurred.</returns>
pvr::Result OpenGLES3DWaifu::initView()
{
...
void OpenGLES3DWaifu::setDefaultOpenglState()
{
	gl::Enable(GL_DEPTH_TEST);
	gl::EnableVertexAttribArray(0);
	gl::EnableVertexAttribArray(1);
	gl::EnableVertexAttribArray(2);
	gl::Enable(GL_CULL_FACE); // 启用剔除操作，即抛弃背面的三角形
	gl::CullFace(GL_BACK);	  // 要剔除的方向
	gl::FrontFace(GL_CCW);	  // 保留的正面方向：逆时针
}
...
/// <returns>Return Result::Success if no error occurred.</returns>
pvr::Result OpenGLES3DWaifu::releaseView()
{
...
/// <returns>Return pvr::Result::Success if no error occurred.</returns>
pvr::Result OpenGLES3DWaifu::renderFrame()
{
...
void OpenGLES3DWaifu::renderNode(uint32_t nodeId, const glm::mat4 &viewProjMatrix, bool &lastRenderWasSkinned)
{
	...
	else
	{
		...
		debugThrowOnApiError("OpenGLES3DWaifu::renderNode Unskinned Setup");
		...
		debugThrowOnApiError("OpenGLES3DWaifu::renderNode Unskinned Set uniforms 1");
		gl::UniformMatrix4fv(_defaultUniformLocations[static_cast<uint32_t>(DefaultUniforms::MVPMatrix)], 1, GL_FALSE, glm::value_ptr(mvp));
		debugThrowOnApiError("OpenGLES3DWaifu::renderNode Unskinned Set uniforms 2");
		...
		debugThrowOnApiError("OpenGLES3DWaifu::renderNode Unskinned Set uniforms 3");

		gl::DrawElements(GL_TRIANGLES, mesh.getNumFaces() * 3, pvr::utils::convertToGles(mesh.getFaces().getDataType()), 0);
		debugThrowOnApiError("OpenGLES3DWaifu::renderNode Unskinned Draw ");
	}
}

/// <summary>This function must be implemented by the user of the shell. The user should return its pvr::Shell object defining the behaviour of the application.</summary>
/// <returns>Return a unique ptr to the demo supplied by the user.</returns>
std::unique_ptr<pvr::Shell> pvr::newDemo() { return std::make_unique<OpenGLES3DWaifu>(); }
